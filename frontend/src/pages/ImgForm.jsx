...
function ImgForm({ auctionId }) {
  ...
  const fetchImages = () => {
    imgService
      .getForAuction(auctionId)
      .then((response) => setImages(response.data))
      .catch(() =>
        toast.current.show({
          severity: "error",
          summary: "Erro",
          detail: "Falha ao carregar imagens.",
        })
      );
  };
  ...
  const customUploader = async (event) => {
    ...
    try {
      ...
      toast.current.show({
        severity: "error",
        summary: "Erro",
        detail: error.response?.data?.message || "Falha no upload.",
      });
    }
  };
  ...
  const handleDelete = async (imageId) => {
    try {
      ...
      toast.current.show({
        severity: "error",
        summary: "Erro",
        detail: "Falha ao remover imagem.",
      });
    }
  };
  ...
  return (
    <div className="image-manager">
      ...
      {images.length > 0 ? (
        <Galleria
          value={images.filter((img) => img?.url && img.url.length > 5)}
          item={itemTemplate}
